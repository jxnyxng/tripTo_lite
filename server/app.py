...
def analyze_with_gemini(survey_data):
    try:
        ...
        prompt = (
            ...
            f"🚨 절대적 예산 제한: 총 예상 비용(항공료 + 숙박비 + 현지 사용 금액)이 {budget}만원을 절대로 초과해서는 안 됩니다. 이는 반드시 지켜져야 하는 필수 조건입니다. "
            f"⚠️ 중요: 추천하기 전에 반드시 항공료 + 숙박비 + 현지 사용 금액을 모두 더해서 {budget}만원 이하인지 확인하세요. 한 항목이라도 예산을 초과하면 해당 여행지는 절대 추천하지 마세요. "
            f"💰 비용 우선순위: 예산 준수가 최우선 조건이므로, 다른 모든 조건보다 예산 내 여행지를 우선으로 추천하세요. "
            ...
            f"비용 계산 검증: 각 추천지의 항공료 + 숙박비 + 현지 사용 금액의 합계가 {budget}만원을 초과하지 않는지 반드시 확인하고, 초과하는 경우 해당 여행지는 추천하지 마세요. "
